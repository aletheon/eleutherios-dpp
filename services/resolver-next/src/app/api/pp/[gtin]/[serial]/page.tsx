...
export default function ProductPage({
  ...
  const btn = {
    base: { padding:"8px 12px", borderRadius:8, background:"#0fa9bd", color:"#0b242d", textDecoration:"none" } as React.CSSProperties
  };
    ...
    <main style={{padding:"2rem", color:"#eaf1f4", background:"#0d2731", minHeight:"100vh", fontFamily:"system-ui"}}>
      ...
      <section style={{background:"#123746", padding:"1rem", borderRadius:12}}>
        ...
        <ul style={{marginTop:16, lineHeight:1.9}}>
          ...
          <li><Link href={`/pp/${gtin}/${serial}/policy`}>Policy (JSON)</Link></li>
          <li><Link href={`/forum/pp/${gtin}/${serial}`}>Forum (UI)</Link></li>
      ...
      <section style={{marginTop:24}}>
        ...
        <div style={{display:"flex", gap:12, flexWrap:"wrap", marginTop:8}}>
          <a href={`/api/actions/repair?gtin=${gtin}&serial=${serial}`} style={btn.base}>Book repair</a>
          <a href={`/api/actions/recycle?gtin=${gtin}&serial=${serial}`} style={btn.base}>Request take-back</a>
        </div>
      </section>
    </main>
  );
